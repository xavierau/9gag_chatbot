"""Note context for MCP server lifespan management.

The NoteContext holds the session factory and user_id for the
...
@dataclass
class NoteContext:
    """Context for note operations within the MCP server.
    ...
    async def get_session(self) -> AsyncGenerator[AsyncSession, None]:
        ...
        Example:
            async with ctx.get_session() as session:
                repo = NoteRepository(session, ctx.user_id)
